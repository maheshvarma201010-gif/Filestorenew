...
async def process_single_post(client: Client, message: Message, reply_text, target_db):
    ...
    share_button = [InlineKeyboardButton("🔁 sʜᴀʀᴇ ᴜʀʟ", url=f'https://telegram.me/share/url?url={link}', style="primary", icon_custom_emoji_id=5440389890787281213)]
    ...
    if is_video(post_message):
        ...
        if settings.get('streaming_active', True):
            ...
            buttons.append([InlineKeyboardButton("🚀 ᴡᴀᴛᴄʜ ᴏɴʟɪɴᴇ", url=stream_url, style="success", icon_custom_emoji_id=5355142851615283756)])
        ...
        if settings.get('download_btn_active', True):
            ...
            buttons.append([InlineKeyboardButton("📥 ᴅᴏᴡɴʟᴏᴀᴅ ꜰɪʟᴇ", url=dl_url, style="success", icon_custom_emoji_id=5355142851615283756)])
