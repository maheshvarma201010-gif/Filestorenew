...
async def send_verify_log(event_type: str, user_id: int, user_obj, session_id: str, verify_link: str, bot_username: str):
    ...
    if event_type == "link_generated":
        ...
        markup = InlineKeyboardMarkup([
            [InlineKeyboardButton("🗑️ Delete Verification", callback_data=f"del_ver_{user_id}_{bot_username}", style="danger")]
        ])
                ...
                try:
                    ...
                    try:
                        ...
                        keyboard = InlineKeyboardMarkup(
                            [[InlineKeyboardButton("⚡️ ˹ ʀᴇᴄᴏᴠᴇʀ ꜰɪʟᴇ ˼ ⚡️", url=reload_url, style="primary")]]
                        )
...
async def short_url(client: Client, message: Message, base64_string, anim_msg=None):
    ...
    if not verification_enabled or verification_method in ["mini_app", "browser"]:
        ...
        buttons = [
            [InlineKeyboardButton("✅ Verify Now", url=final_redirect, style="primary")]
        ]
...
async def proverify_command(client: Client, message: Message):
    try:
        ...
        if not verification_enabled or verification_method in ["mini_app", "browser"]:
            buttons = [
                [InlineKeyboardButton("🔗 ᴘʀᴏ ᴠᴇʀɪꜰʏ", web_app=WebAppInfo(url=final_redirect), style="primary")]
            ]
        else:
            buttons = [
                [InlineKeyboardButton("🔗 ᴘʀᴏ ᴠᴇʀɪꜰʏ", url=final_redirect, style="primary")]
            ]
...
async def verify_command(client: Client, message: Message):
    ...
    if not verification_enabled or verification_method in ["mini_app", "browser"]:
        buttons = [
            [InlineKeyboardButton("🔗 ᴠᴇʀɪꜰʏ / ᴇxᴛᴇɴᴅ", web_app=WebAppInfo(url=final_redirect), style="primary")]
        ]
    else:
        buttons = [
            [InlineKeyboardButton("🔗 ᴠᴇʀɪꜰʏ / ᴇxᴛᴇɴᴅ", url=final_redirect, style="primary")]
        ]

    tut_link = settings.get("tutorial_link", TUT_VID)
    tut_enabled = settings.get("tutorial_enabled", True)
    if tut_enabled and tut_link:
        buttons.append([InlineKeyboardButton("📹 ᴛᴜᴛᴏʀɪᴀʟ", url=tut_link, style="primary")])
...
async def start_command(client: Client, message: Message):
        ...
        if access_status == "banned":
            ...
            return await message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [[InlineKeyboardButton("🛡 ᴄᴏɴᴛᴀᴄᴛ ᴏᴡɴᴇʀ 🛡", url="https://t.me/ALONEKINGSTARBACK", style="primary")]]
                )
            )
    ...
    # Handle normal message flow
        ...
        buttons = [
            [InlineKeyboardButton(f"📢 {bot_name}: JOIN OUR COMMUNITY", url=community_url, style="primary")],
            [
            ...
            [
                ...
                InlineKeyboardButton("🤝 ˹ ʀᴇꜰᴇʀ ˼", callback_data="referral_info", style="primary")
            ]
        ]
...
async def referral_info_cb(client, query: CallbackQuery):
    ...
    await query.message.edit_caption(
        caption=text,
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="start", style="primary")]])
    )
