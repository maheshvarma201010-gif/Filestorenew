...
async def add_admins(client: Client, message: Message):
    ...
    await pro.edit(f"📊 **ᴀᴅᴍɪɴ ᴀᴅᴅ ʀᴇᴘᴏʀᴛ:**\n\n{report}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("ᴄʟᴏsᴇ", callback_data="close", style="danger", icon_custom_emoji_id=5354968347094046619)]]))
...
async def fsub_bot_manager(client: Client, message: Message):
    ...
    if not bots:
        ...
        for bot in bots:
            ...
            buttons.append([InlineKeyboardButton(f"🗑 Delete {bot['name']}", callback_data=f"dfbot_{bot['_id']}", style="danger", icon_custom_emoji_id=5354968347094046619)])

    buttons.append([InlineKeyboardButton("➕ Add New Bot", callback_data="afbot", style="success", icon_custom_emoji_id=5355142851615283756)])
    buttons.append([InlineKeyboardButton("❌ Close", callback_data="close", style="danger", icon_custom_emoji_id=5354968347094046619)])
...
async def reset_verification(client: Client, message: Message):
    ...
    if arg_str.lower() == "all":
        try:
            ...
            return await pro.edit(report, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Close", callback_data="close", style="danger", icon_custom_emoji_id=5354968347094046619)]]))
        except Exception as e:
    ...
    await pro.edit(f"📊 **ʀᴇsᴇᴛ ʀᴇᴘᴏʀᴛ:**\n\n{report}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Close", callback_data="close", style="danger", icon_custom_emoji_id=5354968347094046619)]]))
...
async def check_redirects_cmd(client: Client, message: Message):
    ...
    if report:
        await pro.edit(report, disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cʟᴏsᴇ", callback_data="close", style="danger", icon_custom_emoji_id=5354968347094046619)]]))
    else:
        await pro.delete()
...
async def save_command(client: Client, message: Message):
    ...
    buttons = [
        [
            InlineKeyboardButton("Backup", callback_data="system_backup", style="success", icon_custom_emoji_id=5355142851615283756),
            InlineKeyboardButton("Restore", callback_data="system_restore", style="success", icon_custom_emoji_id=5355142851615283756)
        ],
        [InlineKeyboardButton("Close", callback_data="close", style="danger", icon_custom_emoji_id=5354968347094046619)]
    ]
...
async def db_channel_manager(client: Client, message: Message):
    ...
    if not channels:
        ...
        for cid in channels:
            ...
            buttons.append([InlineKeyboardButton(f"🗑 Delete {cid}", callback_data=f"ddbch_{cid}", style="danger", icon_custom_emoji_id=5354968347094046619)])

    buttons.append([InlineKeyboardButton("➕ Add DB Channel", callback_data="adbch", style="success", icon_custom_emoji_id=5355142851615283756)])
    buttons.append([InlineKeyboardButton("❌ Close", callback_data="close", style="danger", icon_custom_emoji_id=5354968347094046619)])
        ...
        if not channels:
            ...
            for chid in channels:
                try:
                    ...
                    text += f"<b>• Unknown Channel</b>\n<code>ID: {chid}</code>\n\n"
                buttons.append([InlineKeyboardButton(f"🗑 Delete {chid}", callback_data=f"ddbch_{chid}", style="danger", icon_custom_emoji_id=5354968347094046619)])
        buttons.append([InlineKeyboardButton("➕ Add DB Channel", callback_data="adbch", style="success", icon_custom_emoji_id=5355142851615283756)])
        buttons.append([InlineKeyboardButton("❌ Close", callback_data="close", style="danger", icon_custom_emoji_id=5354968347094046619)])
        await query.message.edit_text(text, reply_markup=InlineKeyboardMarkup(buttons))
