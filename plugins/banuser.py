...
async def ban_user_cmd(client: Client, message: Message):
    ...
    await pro.edit(f"📊 **Bᴀɴ Rᴇᴘᴏʀᴛ:**\n\n{report}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cʟᴏsᴇ", callback_data="close", style="danger")]]))
...
async def ban_list_cmd(client: Client, message: Message):
    ...
    if not banuser_ids:
        return await pro.edit("✨ <b>˹ ʙᴀɴ ʟɪsᴛ ᴇᴍᴘᴛʏ ˼</b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔒 ᴄʟᴏsᴇ", callback_data="close", style="danger")]]))
    ...
    await pro.edit(result, disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cʟᴏsᴇ", callback_data="close", style="danger")]]))
