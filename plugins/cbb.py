...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if data == "help":
        ...
        buttons = [
            [InlineKeyboardButton('🔙 ʙᴀᴄᴋ ᴛᴏ ʜᴏᴍᴇ', callback_data='start', style="primary", icon_custom_emoji_id=5440389890787281213)],
            [InlineKeyboardButton('🛡️ ᴄʟᴏsᴇ ᴍᴇɴᴜ', callback_data='close', style="danger", icon_custom_emoji_id=5354968347094046619)]
        ]
        try:
        ...
        buttons = [
            [InlineKeyboardButton("📢 NAME: JOIN OUR COMMUNITY", url="https://t.me/AniZoneFlix", style="primary", icon_custom_emoji_id=5440389890787281213)],
            [
...
#
        ...
        await send_media(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [InlineKeyboardButton("💬 Cᴏɴᴛᴀᴄᴛ Aᴅᴍɪɴ", url=SCREENSHOT_URL, style="primary", icon_custom_emoji_id=5440389890787281213)],
                    [InlineKeyboardButton("🔒 Cʟᴏsᴇ", callback_data="close", style="danger", icon_custom_emoji_id=5354968347094046619)]
                ]
            )
        )
        ...
        try:
            ...
            buttons = [
                [InlineKeyboardButton(f"ʀᴇǫ ᴍᴏᴅᴇ {'OFF' if mode == 'on' else 'ON'}", callback_data=f"rfs_toggle_{cid}_{new_mode}", style="primary", icon_custom_emoji_id=5440389890787281213)],
                [InlineKeyboardButton("‹ ʙᴀᴄᴋ", callback_data="fsub_back", style="primary", icon_custom_emoji_id=5440389890787281213)]
            ]
            await query.message.edit_text(
        ...
        for cid in channels:
            try:
                ...
                buttons.append([InlineKeyboardButton(f"{status} {chat.title}", callback_data=f"rfs_ch_{cid}", style="primary", icon_custom_emoji_id=5440389890787281213)])
            except:
                continue
