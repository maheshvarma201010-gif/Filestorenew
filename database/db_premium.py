...
# In-memory premium status cache
_PREMIUM_CACHE = {}
_CACHE_TTL = 30  # 30 seconds cache TTL for premium status

# Check if the user is a premium user
async def is_premium_user(user_id):
    now = time.time()
    if user_id in _PREMIUM_CACHE:
        val, expiry = _PREMIUM_CACHE[user_id]
        if now < expiry:
            return val

    user = await collection.find_one({"user_id": user_id})  # Async query
    is_prem = user is not None
    _PREMIUM_CACHE[user_id] = (is_prem, now + _CACHE_TTL)
    return is_prem

# Remove premium user
async def remove_premium(user_id):
    await collection.delete_one({"user_id": user_id})  # Async removal
    _PREMIUM_CACHE[user_id] = (False, time.time() + _CACHE_TTL)
    ...
    async for user in collection.find({}):
        ...
        try:
            ...
            if expiration_time <= current_time:
                ...
                # Invalidate cache
                _PREMIUM_CACHE.pop(user["user_id"], None)
...
async def add_premium(user_id, time_value, time_unit):
    ...
    # Update cache
    _PREMIUM_CACHE[user_id] = (True, time.time() + _CACHE_TTL)
